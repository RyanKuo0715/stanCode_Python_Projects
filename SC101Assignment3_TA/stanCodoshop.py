...
def get_best_pixel(pixels):
    ...
    dist_list = []
    for pixel in pixels:
        dist_list.append((get_pixel_dist(pixel, avg[0], avg[1], avg[2]), pixel))
    return min(dist_list, key=lambda t: t[0])[1]
    # # code 2:
    # return min(pixels, key=lambda pixel: get_pixel_dist(pixel, avg[0], avg[1], avg[2]))
...
def solve(images):
    ...
    for i in range(width):
        for j in range(height):
            # find the best pixel among images
            best_pixel = get_best_pixel(list(image.get_pixel(i, j) for image in images))
            # set the best pixel
            # code 1
            ...
            # # SimpleImage Guide: https://web.stanford.edu/class/archive/cs/cs106ap/cs106ap.1198/handouts/h10_Image_Reference_Guide.pdf
