...
def simplify_dict(simple_dict, dictionary, words):
	...
	for ele in dictionary:
		if ele[0] in word_num and len(ele) >= 4:
			illegal = False
			for ch in ele:
				if ch not in word_num or ele.count(ch) > word_num[ch]:
					illegal = True
					break
			if not illegal:
				simple_dict.append(ele)
...
def get_boggle_helper(position, simple_dict, words, find_word, path, boggle):
	...
	if position not in path and has_prefix(simple_dict, find_word):
		# choose
